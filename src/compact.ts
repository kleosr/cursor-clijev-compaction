import { noulAnswer } from './jev.js';
import { collectToolCalls, fitState } from './state.js';
import { estimateTokens as tokenCount } from './tokens.js';
import type {
  CallAnswer,
  CallDecision,
  CompactOptions,
...
export function questionsFor(call: ToolCall): JevQuestions {
  return {
    [`call_${call.id}`]: {
      ...
      criteria: {
        true: 'The call and its arguments are still needed for later work',
        false: 'The call can be dropped; the assistant can re-run it if needed',
      },
    },
    [`result_${call.id}`]: {
      type: 'noul',
      ...
      criteria: {
        true: 'The verbatim result is still required and re-running would not replace it',
        false: 'The result can be truncated or dropped; re-run the tool if needed',
      },
    },
  };
}
...
export function batchCalls(
  ...
  for (const call of calls) {
    const tokens = tokenCount(JSON.stringify(questionsFor(call)));
...
function truncatedResultText(text: string, isError: boolean, headChars: number): string {
  ...
  return `${head}[cursor-jev truncated ${text.length - headChars} chars of this tool result${
    isError ? ' (error)' : ''
